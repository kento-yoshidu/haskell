...
今回はhtmlファイルの大枠、html要素、head要素、body要素について学習します。

前提として、`index.html`は以下のようになっているはずです。この内容をもとに、それぞれの要素を解説していきます。もし違っていれば以下のように書き換えることをお勧めします。
...
などなど。どれもブラウザや検索エンジンが利用する情報で、ユーザの目には直接触れない情報です（ページのタイトルだけは例外ですが）。
...
meta要素とは、文字通り**ページのメタ情報**を記述するためのものです。


## title属性

**title属性**は、その名の通りページのタイトルを表します。`index.html`のtitle属性を以下のように書き換えてみます。
